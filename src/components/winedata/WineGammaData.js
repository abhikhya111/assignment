...
export default function WineGammaData() {
    ...
    // Function to calcutate the gamma value of mean median and mode based on class( Alcohal)
    function calculateClassStatistics(dataset) {
        const classStatistics = {};

        for (const data of dataset) {
            const { Alcohol, Ash, Hue, Magnesium } = data;
            if (!classStatistics[Alcohol]) {
                classStatistics[Alcohol] = [];
            }
            classStatistics[Alcohol].push((Ash * Hue) / Magnesium);
        }
        const newData = [];
    ...
    return (
        <div>
            <h4> Property “Gamma” for each point of the dataset. “Gamma” can be 
                 calculated as Gamma = (Ash * Hue) / Magnesium.</h4>
